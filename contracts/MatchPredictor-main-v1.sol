...
/// @title MatchPredictor (main v1 — versione destinata a mainnet)
...
///
/// @dev Basato su MatchPredictor-v3.sol (versione testnet), con un fix di sicurezza
///      aggiuntivo emerso da una revisione pre-mainnet:
///      - reportResult() ora richiede che la deadline sia passata prima di accettare
///        un risultato (nuovo errore PredictionWindowStillOpen).
///      - predictFor()/predictBatchFor() controllano esplicitamente anche m.resolved,
///        non solo la deadline — seconda linea di difesa indipendente, nel caso deadline
///        e risoluzione si disallineassero per un bug futuro dell'oracolo.
///
///      Cambiamenti ereditati dalla v3 rispetto alla v2:
...
contract MatchPredictor is LSP8Mintable {
    ...
    error EmptyBatch();
    error PredictionWindowStillOpen();
    ...
    function predictFor(
        ...
        // Ridondante nella normale operatività (una partita risolta ha quasi sempre
        // anche la deadline passata), ma è una seconda linea di difesa indipendente:
        // se mai la deadline e la risoluzione si disallineassero, questo blocca comunque
        // un pronostico "a colpo sicuro" su un risultato già noto.
    ...
    function predictBatchFor(
        ...
        for (uint256 i = 0; i < matchIds.length; i++) {
            ...
            if (m.resolved) {
                emit PredictionSkipped(matchId, predictor, "MatchAlreadyResolved");
                continue;
            }
    ...
    function reportResult(uint256 matchId, Result actualResult) external onlyOracle {
        ...
        if (!m.exists) revert MatchDoesNotExist();
        if (m.resolved) revert MatchAlreadyResolved();
        // Difesa aggiuntiva: non ci si fida ciecamente della disciplina dell'oracolo
        // off-chain nel riportare solo dopo la fine reale della partita — se per bug
        // o chiave compromessa arrivasse un risultato troppo presto, si aprirebbe una
        // finestra in cui si potrebbe pronosticare conoscendo già l'esito.
        if (block.timestamp < m.predictionDeadline) revert PredictionWindowStillOpen();
